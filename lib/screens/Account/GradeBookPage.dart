...
class _GradeBookPageState extends State<GradeBookPage> {
  ...
  List<String>? returnList(){
    ...
    for(int i =0; i < widget.gradebook[0].student!.lessons!.length; i++) {
      ...
      for(int j =0; j < subjAbbrev.length; j++){
        if(widget.gradebook[0].student!.lessons![i].lessonNameAbbrev == subjAbbrev[j]){
          isHere = true;
          break;
        }
      }
      if(isHere == false){
        subjAbbrev.add(widget.gradebook[0].student!.lessons![i].lessonNameAbbrev!);
      }
    }
    return subjAbbrev;
  }
  ...
  List<LessonDto>? fillList(List<String>? abbrev){
    ...
    for(int j =0; j < abbrev!.length; j++){
      ...
      for(int i =0; i < widget.gradebook[0].student!.lessons!.length; i++){
        if(widget.gradebook[0].student!.lessons![i].lessonNameAbbrev == abbrev[j]){
          if(widget.gradebook[0].student!.lessons![i].lessonTypeAbbrev == "ПЗ"){
            int mark = 0;
            ...
            controlPointPz.add(SubjectType(
                widget.gradebook[0].student!.lessons![i].dateString != null
                    ...
                    :"",
                widget.gradebook[0].student!.lessons![i].gradeBookOmissions != null
          ...
          else if(widget.gradebook[0].student!.lessons![i].lessonTypeAbbrev == "ЛК"){
            int mark = 0;
            if(widget.gradebook[0].student!.lessons![i].marks!.isNotEmpty) {
              mark = widget.gradebook[0].student!.lessons![i].marks![0];
            }
          ...
          else if(widget.gradebook[0].student!.lessons![i].lessonTypeAbbrev == "ЛР"){
            int mark = 0;
              if(widget.gradebook[0].student!.lessons![i].marks!.isNotEmpty) {
              mark = widget.gradebook[0].student!.lessons![i].marks![0];
            }
      ...
      result.add(LessonDto(abbrev[j], ControlPoint(controlPointPz, controlPointLc, controlPointLw)));
    }
    return result;
  }

  List<int>? marksByLesson(int n, List<LessonDto> lesson, int typeOfSubject, String controlPoint) {
  ...
  int? omissionsByLesson(int n, List<LessonDto> lesson, int typeOfSubject, String controlPoint) {
    int result = 0;
    switch(typeOfSubject) {
      case 1:
        if (lesson[n].controlpoint!.pz!.isNotEmpty) {
